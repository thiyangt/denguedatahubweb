...
ggplot( aes(y=mean.longitude, x=incidence)) + geom_point(alpha=0.5) +
  scale_fill_continuous(type = "viridis") +
  theme_bw()+ facet_wrap(~year) + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

sum.ll %>% filter(0 < incidence & incidence < 100000000) %>%
ggplot( aes(x=mean.latitude, y=incidence)) + geom_point(alpha=0.5) +
  scale_fill_continuous(type = "viridis") +
  theme_bw()+ facet_wrap(~year) + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
```